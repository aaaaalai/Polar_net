...
#define N_ENDPOINT 2
#define PROP_SIZE (N_ENDPOINT + N_OFFSETS)
...
__device__ inline bool devIoU(scalar_t const * const a, scalar_t const * const b, const float threshold) {
  const int start_a = (int) (a[0] * N_STRIPS + 0.5);
  const int start_b = (int) (b[0] * N_STRIPS + 0.5);
  const int start = max(max(start_a, start_b), 0);
  // const int end_a = start_a + a[4] - 1 + 0.5 - ((a[4] - 1) < 0);
  // const int end_b = start_b + b[4] - 1 + 0.5 - ((b[4] - 1) < 0);
  const int end_a = (int) (a[1]*N_STRIPS);
  const int end_b = (int) (b[1]*N_STRIPS);
  const int end = min(min(end_a, end_b), N_OFFSETS - 1);
  if (end < start) return false;
  scalar_t dist = 0;
  for(unsigned char i = N_ENDPOINT + start; i <= N_ENDPOINT + end; ++i) 
  {
