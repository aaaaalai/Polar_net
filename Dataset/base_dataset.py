...
class BaseTrSet(Dataset):
    def __init__(self, cfg=None, transforms=None):
        ...
        self.train_augments = A.Compose(img_transforms,
    ...
    def __getitem__(self, index):
        ...
        if num_lanes > 0:
            ...
            _, lane_point_xs, lane_point_validmask, end_points, lane_dense_sample_car = self.fit_lane(lanes_car,
                                                                                                      is_sample=True)
            line_paras, _ = self.curve2line_group(lane_point_xs, lane_point_validmask, 1)
            line_paras = np.squeeze(line_paras, axis=1)
            line_paras_group, group_valid_masks = self.curve2line_group(lane_point_xs, lane_point_validmask,
                                                                        self.num_line_groups)
    ...
    def get_polar_map(self, xy_car):
        ...
        grid_x, grid_y = np.meshgrid(np.arange(0.5, self.polar_map_size[1], 1, dtype=np.float32),
                                     np.arange(0.5, self.polar_map_size[0], 1, dtype=np.float32))
        grid_x, grid_y = grid_x.flatten() / self.polar_map_size[1] * self.img_w, grid_y.flatten() / self.polar_map_size[
            0] * self.img_h
