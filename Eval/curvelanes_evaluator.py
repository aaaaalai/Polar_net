...
def deresize_output(lanes, img_shape, cut_height, ocut_height, ori_img_h):
    ...
    for lane in lanes:
        lane['points'][..., 1] = (
            (lane['points'][..., 1] * ori_img_h - cut_height)
            / (ori_img_h - cut_height)
            * (oimg_h - ocut_height)
            + ocut_height
        ) / oimg_h
...
class CurveLanesEvaluator(BaseEvaluator):
    ...
    def pre_process(self):
        if self.is_view:
            ...
            os.makedirs(os.path.join(self.view_path, 'labels'), exist_ok=True)
        else:
        ...
        with open(self.txt_path, 'r') as file_list:
            ...
            self.label_name_list = [
                img_name.replace('.jpg', '.lines.json').replace('images', 'labels')
    ...
    def write_output(self, outputs, file_names_with_shape):
        ...
        save_label_dir = os.path.join(self.result_path, 'labels')
        ...
        for lanes, file_name_with_shape in zip(lanes_list, file_names_with_shape):
            ...
            # 关键修复：不要再依赖原始 file_name 的目录结构
            # 统一只取文件名，保存到 RESULTS/labels/xxx.lines.json
            base_name = os.path.basename(file_name)
            base_name = os.path.splitext(base_name)[0] + '.lines.json'
            out_path = os.path.join(save_label_dir, base_name)

            lanes = deresize_output(
                lanes,
                ori_img_shape,
                self.cut_height,
                self.cut_height_dict[ori_img_shape],
                self.ori_img_h
            )

            write_output_curvelanes_format(
                lanes,
                ori_img_shape,
                out_path,
                step_size=1
    ...
    def view_output(self, outputs, file_names_with_shape, ori_imgs):
        ...
        for lanes, line_paras, file_name_with_shape, ori_img in zip(
            lanes_list, line_paras_batch, file_names_with_shape, ori_imgs
        ):
            file_name, ori_img_shape = file_name_with_shape
            ...
            base_name = os.path.basename(file_name)
            out_path_pred = os.path.join(self.view_path, base_name.replace('.jpg', '_pred.jpg'))
            out_path_anchor = os.path.join(self.view_path, base_name.replace('.jpg', '_anchor.jpg'))

            os.makedirs(os.path.dirname(out_path_pred), exist_ok=True)
            os.makedirs(os.path.dirname(out_path_anchor), exist_ok=True)

            plot_img = super().view_single_img_lane(ori_img, lanes)
            cv2.imwrite(out_path_pred, plot_img)

            plot_img = self.view_single_img_line(
                ori_img, line_paras, self.cut_height_dict[ori_img_shape]
            )
            cv2.imwrite(out_path_anchor, plot_img)

    def view_single_img_line(self, img, line_paras, cut_height):
        plot_img = img.copy()
        plot_img = self.ploter.plot_lines_oriimg_unfix(
            plot_img, line_paras, cut_height, color=(72, 195, 238)
        )
        return plot_img
    ...
    def evaluate_valset(self, is_sequential=False):
        ...
        annotations = load_data_curvelanes_format(
        ...
        if is_sequential:
            ...
            with Pool(cpu_count()) as p:
                results = p.starmap(
                    evaluate_core,
                    zip(pred_lanes_list, gt_lanes_list, ori_img_shape_list, repeat(eval_params))
                )
    ...
    def view_gt(self):
        for img_name, label_name in tzip(
            self.img_name_list,
            self.label_name_list,
            desc='Ploting grounding truth'
        ):
            ...
            out_path = os.path.join(self.view_path, base_name.replace('.jpg', '_gt.jpg'))
